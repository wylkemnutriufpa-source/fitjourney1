    ...
    <>
      {patientId && (
        <div className="sticky top-0 z-30 border-b border-[var(--gold)]/40 bg-[color-mix(in_oklab,var(--gold)_8%,var(--background))] backdrop-blur">
          <div className="mx-auto w-full max-w-5xl px-6 py-2.5 flex items-center justify-between gap-3 flex-wrap">
            <p className="text-xs font-mono uppercase tracking-wider text-[var(--gold)] flex items-center gap-2">
              <UserCheck className="size-3.5" />
              Comparando com <span className="font-bold">{patientName ?? "paciente"}</span>
            </p>
            <Link
              to="/patients/$id"
              params={{ id: patientId }}
              className="text-[10px] font-mono uppercase tracking-wider text-muted-foreground hover:text-[var(--gold)] flex items-center gap-1"
            >
